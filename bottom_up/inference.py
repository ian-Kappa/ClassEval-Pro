...
class HierarchicalCodeGenPipeline:
    ...
    2. Generate code bottom-up based on dependency tree
    """
...
- Order methods by level (ascending: 0, 1, 2, ...)
...
"""
    ...
    def construct_dependency_based_prompt(self, skeleton, dependency_tree, class_name, greedy=False):
        """
        Stage 2: Generate code using dependency-aware bottom-up approach
        """
        ...
        # Extract method order from dependency tree
        sorted_methods = sorted(dependency_tree.items(), key=lambda x: x[1]['level'])
...
# Goal
Implement ALL methods of `{class_name}` following the bottom-up dependency order to ensure correctness.
...
1. **Bottom-up implementation**: Implement Level 0 methods first, then Level 1, etc.
2. **Dependency satisfaction**: When implementing a method at Level N, ALL its dependencies (from `depends_on` list) must already be complete
3. **No forward references**: Never call a method before it's fully implemented
...
# Workflow
1. **Start with Level 0**: Implement all independent methods first
2. **Progress by level**: Move to Level 1 only after Level 0 is complete, etc.
3. **Verify dependencies**: Before implementing a method, confirm all its dependencies are implemented
...
# All methods implemented following dependency order: {method_order_str}
...
Return only the complete implementation in a Python code block."""
                ...
                # Stage 2: Bottom-up Code Generation
